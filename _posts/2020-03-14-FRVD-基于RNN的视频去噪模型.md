...
This code is modified from [EDSR-pytorch](https://github.com/thstkdgus35/EDSR-PyTorch) github repository
for video denosing.

This method is adpated from CVPR2018 paper : [Frame-Recurrent Video Super-Resolution](https://arxiv.org/abs/1801.04590)
...
**网络的整体框架如下**：首先通过计算带噪视频两帧$(I_{t-1}, I_t)$之间的光流图 $F$，然后将光流图 $F$ 作用在前帧的预测结果 $I^{est}_{t-1}$中得到 $\hat{I}^{est}_{t-1}$，然后与$I_t$一起输入到去噪网络中得到去噪结果。
...
### 4. 改进思路
1. RNN模型只能考虑前一帧的信息，设计网络融合前n帧的信息。
